...
const updateContacts = async () => {
  ...
  // go thru every email
  emailsTable.select({
    // // email record ID for testing:
    // maxRecords: 100,
    // filterByFormula: `SEARCH(RECORD_ID(), "xxx")`,
    // view: '_test',
    // filterByFormula: `SEARCH(RECORD_ID(), "xxx, yyy, zzz")`,
  })
    .eachPage(
      async function page(emailRecs, fetchNextPage) {
        for (let i = 0; i < emailRecs.length; i++) {
          ...
          // // Updating SendinBlue contacts for all members' email addresses in Airtable, regradless if they have been verified, ie, been used to log in
